...
function clear() {
    for (i = 0; i < 42; i++) {
        ...
        $("#GD" + i).removeClass("selday");//selday
...
function mOck(thisObj, v,i){
	...
	if (type=="workday" || type=="seldaywork") {  //特殊工作日
		if (onoff == "0") {
			...
			document.getElementById("setholiday").innerHTML += "<span class='date' id="+ymd+">"+ymd+"   "+"<img id='imgclick' value='"+ymd+"' onClick='imgonclick("+ymd+","+ids+","+lx+")' src='images/false.png'/></span>";//将内容加入框中
		...
		if(parseInt(sysTime) < parseInt(ymd)){
			ymd = ymd.substring(0,4)+"-" + ymd.substring(4,6)+"-"+ymd.substring(6,8);
			document.getElementById("setholiday").innerHTML += "<span  class='date' id="+ymd+">"+ymd+"   "+"<img id='imgclick' value='"+ymd+"' onClick='imgonclick("+ymd+","+ids+","+lx+")' src='/img/false.png'/></span>";//将内容加入框中
		}else{
			swal({title: "",   text: "此日期不可选择!", type:'error' ,  timer:1000, showConfirmButton: false });
			return false;
		}
			...
			hDays.push(dayJson);//加入数组
			thisObj.setAttribute("class", "");//设定为选中样式  selday
		}else if (onoff == "1"){
			thisObj.attributes["on"].value='0';
			if(document.getElementById(ymd)){
				$("#"+ymd).remove();
...
var dateSelection = {
    ...
    showYearContent : function(beginYear, endYear) {
        ...
        var s = '';
        for (var i = this.beginYear; i <= 2018; i++) {
